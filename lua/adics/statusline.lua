...
local function file_read_only()
	...
	if is_readonly then
		return color("StatuslineBoolean", "[RO]")
	end
...
local function lsp_status()
	...
	local messages = {}
	if #errors ~= 0 then
		table.insert(messages, color("StatuslineError", string.format("Error [%s]", #errors)))
	end
	if #warnings ~= 0 then
		table.insert(messages, color("StatuslineWarning", string.format("Warn [%s]", #warnings)))
	end
